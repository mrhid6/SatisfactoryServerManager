...
class NotificationHandler {
    ...
    TriggerNotification = async (Notification) => {
        ...
        const NotifyInterface = this._notifyinterfaces.find(int => int.getId() == Notification.get("webhook_id"))
    ...
    GetPendingEvents = async () => {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (Notification.get("handled") == false && Notification.get("attempts") < 10) {
    ...
    PurgeEvents = async () => {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (Notification.get("handled") == true && Notification.get("timestamp") < limitDate.getTime()) {
                await DB.queryRun("DELETE FROM webhook_events WHERE we_id=?", [Notification._id])
            }
        }
    }
