...
class SF_Server_Handler {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.SteamCMD.download()
                ...
                .then(() => {
                    ...
                    logger.info(
                        "[SFS_Handler] - Installed/Validated SteamCmd binaries"
                    );
    ...
    FixSteamCmdPerms() {
        return new Promise((resolve, reject) => {
            if (platform == "linux") {
                const steamcmdexe = path.join(
                    Config.get("ssm.steamcmd"),
                    "steamcmd.sh"
                );
        ...
        return new Promise((resolve, reject) => {
            this.SteamCMD.getAppInfo(1690800)
                .then((data) => {
                    ...
                    if (
                        ServerVersion >
                            Config.get("satisfactory.server_version") &&
                        Config.get("satisfactory.updateonstart") == true
                    ) {
                        logger.info(
                            "[SFS_Handler] - SF Server Requires Updating.."
                        );
    ...
    InstallSFServer = async (forceInstall = false) => {
    ...
    _RemoveSFServer = async () => {
        ...
        try {
            rimraf.sync(Config.get("satisfactory.server_location"));
    ...
    _InstallSFServer = async () => {
        ...
        if (installPath.indexOf(" ") > -1) {
            logger.error(
                "[SFS_Handler] - Install path must not contain spaces!"
            );
    ...
    execSFSCmd(command) {
        ...
        return new Promise((resolve, reject) => {
            ...
            process.on("error", (err) => {
                logger.debug(
                    `[SFS_Handler] [SERVER_ACTION] - Child process with error ${err}`
                );
            });
            process.on("close", (code) => {
                logger.debug(
                    `[SFS_Handler] [SERVER_ACTION] - Child process on close ${code}`
                );
            });
    ...
    startServer() {
        return new Promise((resolve, reject) => {
            logger.debug(
                "[SFS_Handler] [SERVER_ACTION] - SF Server Starting ..."
            );
            this._getServerState()
                .then((server_status) => {
                    if (server_status.pid1 == -1 && server_status.pid2 == -1) {
                        ...
                        const { ServerQueryPort, BeaconPort, Port } =
                            this.getStartupPortConfig();

                        const workerThreads = Config.get(
                            "satisfactory.worker_threads"
                        );
                        return this.execSFSCmd(
                            `?listen -Port=${Port} -ServerQueryPort=${ServerQueryPort} -BeaconPort=${BeaconPort} -unattended -MaxWorkerThreads=${workerThreads}`
                        );
                    } else {
                        logger.debug(
                            "[SFS_Handler] [SERVER_ACTION] - SF Server Already Running"
                        );
                ...
                .then((res) => {
                    this._getServerState();
                    this.wailTillSFServerStarted()
                        .then(() => {
                            logger.debug(
                                "[SFS_Handler] [SERVER_ACTION] - SF Server Started"
                            );
                            resolve(res);
                        })
                        .catch((err) => {
                            logger.warn(
                                "[SFS_Handler] [SERVER_ACTION] - SF Server Failed To Start"
                            );
                            reject(err);
                        });
                })
            ...
            this._getServerState().then((server_status) => {
                if (server_status.pid1 != -1 && server_status.pid2 != -1) {
                    ...
                    this.wailTillSFServerStopped().then(() => {
                        logger.debug(
                            "[SFS_Handler] [SERVER_ACTION] - SF Server Stopped"
                        );
        ...
        return new Promise((resolve, reject) => {
            this._getServerState().then((server_status) => {
                if (server_status.pid1 != -1 && server_status.pid2 != -1) {
                    ...
                    this.wailTillSFServerStopped().then(() => {
                        logger.debug(
                            "[SFS_Handler] [SERVER_ACTION] - SF Server Killed"
                        );
                        Cleanup.decreaseCounter(1);
                        this._getServerState().then(() => {
                            resolve();
        ...
        return new Promise((resolve, reject) => {
            ...
            this.isGameInstalled()
                ...
                .then((data) => {
                    ...
                    let process1 = data.list.find((el) =>
                        el.params.includes(
                            Config.get("satisfactory.server_exe")
                        )
                    );
                    let process2 = data.list.find(
                        (el) =>
                            el.name == Config.get("satisfactory.server_sub_exe")
                    );
    ...
    validateSaveFile(savefile) {
        return new Promise((resolve, reject) => {
            this.readSaveFileOffset(savefile, 38, 1)
                .then((buffer) => {
    ...
    getSaveVersion(savefile) {
        return new Promise((resolve, reject) => {
            this.readSaveFileOffset(savefile, 4, 4)
                .then((buffer) => {
    ...
    getSaveBodyLength(savefile) {
        return new Promise((resolve, reject) => {
            this.readSaveFileOffset(savefile, 0, 4)
                .then((buffer) => {
                    resolve(buffer.readUInt8());
                })
                .catch((err) => {
    ...
    getSaveBodyString(savefile, length) {
        return new Promise((resolve, reject) => {
            this.readSaveFileOffset(savefile, 15, 100)
                .then((buffer) => {
    ...
    deleteSaveFile(savefile) {
        return new Promise((resolve, reject) => {
            ...
            if (fs.existsSync(SaveFile) == false) {
                logger.error(
                    "[SFS_Handler] - Remove Save Error: Save File Doesn't Exist!"
                );
